...
const getPlace = async (req, res, next) => {
  ...
  try {
    ...
    if (places.length > 0) {
      ...
      return res.success(messages.loactionDataFetched, results[0]);
    }
    return res.error(
...
const placeSearch = async (req, res, next) => {
  ...
  try {
    ...
    if (places.features && places.features.length > 0) {
      ...
      return res.success(messages.placeFound, results);
    } else {
      //   return res
      //     .status(responseStatusCodes.notFound)
      //     .json({ message: "No places found" });
      return res.error(
