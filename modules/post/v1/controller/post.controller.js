    ...
    try {
        ...
        if (images && images.length > 0) {
            ...
            const textSvg = `
                ...
                <text x="50%" y="45%" class="title">${escapeXml(nameText)}</text>
                </svg>
            `;
            ...
            const buffer = await base
                .composite([
                { input: Buffer.from(logoSvg), top: 0, left: 0 },
                ])
                .jpeg({ quality: 92 });
...
exports.deletAdImage = async (req, res) => {
    ...
    try {
        ...
        if (!data) {
            return res.status(responseStatusCodes.notFound).json({ success: false, message: responseMessages.imageNotFound });
        }
        await deleteImageFromS3(data.image);
        await AdImage.destroy({ where: { id } });
        return res.status(responseStatusCodes.success).json({ success: true, message: responseMessages.imageDeleted });
    } catch (err) {
        return res.status(responseStatusCodes.internalServerError).json({ success: false, message: responseMessages.internalServerError });
    }
}
...
exports.updateAdAddress = async (req, res) => {
    ...
    try {
        ...
        return res.status(responseStatusCodes.success).json({ success: true, message: responseMessages.locationSuccess,successCount: response.successCount, failureCount: response.failureCount });
    } catch (err) {
...
exports.deleteAd = async (req, res) => {
    ...
    try {
        ...
        if (!adRows) {
            return res.status(responseStatusCodes.notFound).json({ message: responseMessages.adDeleted });
        }
        ...
        return res.status(responseStatusCodes.success).json({ message: responseMessages.adDeleted });
    } catch (error) {
...
exports.getAdDetails = async (req, res) => {
    try {
        ...
        if (!ad) {
            return res.status(responseStatusCodes.notFound).json({ message: responseMessages.adNotFound });
        }
        if(userId){
    ...
    try {
        ...
        if (req.body.id) {
            const blockedRecords = await BlockedUser.findAll({
                where: {
                    [Op.or]: [
                        { blocker_id: req.body.id },
                        { blocked_id: req.body.id }
                    ]
                },
                raw: true
            });
            ...
            blockedUserIds = blockedRecords.map(record =>
                record.blocker_id !== req.body.id ? record.blocked_id : record.blocker_id
            );
        }
...
exports.addToWishlist = async (req, res) => {
    try {
        ...
        if (!ad_id) {
            return res.status(responseStatusCodes.notFound).json({ message: responseMessages.invalidRequest });
        }
        const userId = req.user.id;
        ...
        if (wishList) {
            ...
            return res.status(responseStatusCodes.success).json({ success: true, message: responseMessages.wishlistRemoved });
        } else {
            await AdWishLists.create({
                user_id: userId,
                ad_id: ad_id
            });
            return res.status(responseStatusCodes.success).json({ success: true, message: responseMessages.wishlistAdded });
        }
    } catch (error) {
        res.status(responseStatusCodes.internalServerError).json({ message: responseMessages.internalServerError });
    }
