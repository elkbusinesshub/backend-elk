...
const createAd = async (req, res, next) => {
  try {
    ...
    console.log(usersToNotify.map((user) => user.notification_token));
    ...
    // });
    return res.success(messages.adCreated, {
      message: messageDisplay,
      ad_id: adId,
      successCount: response.successCount,
      failureCount: response.failureCount,
...
const updateAdImage = async (req, res, next) => {
  ...
  try {
    ...
    if (images || images.length !== 0) {
      for (const image of images) {
        ...
        if (!uploaded) {
          return res.error(messages.imageUploadFailed);
        }
        adImages.push({
          ad_id: ad_id,
          image: fileName,
    ...
    //   data: updatedImages,
    // });
    return res.success(messages.imageUploadSuccess, updatedImages);
  } catch (err) {
    return next(err);
  }
};

const deletAdImage = async (req, res, next) => {
  const { id } = req.body;
  try {
    const data = await AdImage.findOne({ where: { id } });
    if (!data) {
      return res.error(messages.imageNotFound);
    }
    await deleteImageFromS3(data.image);
    await AdImage.destroy({ where: { id } });
    return res.success(messages.imageDeleted);
  } catch (err) {
    return next(err);
  }
};

const updateAdAddress = async (req, res, next) => {
  const {
    ad_id,
    country,
    latitude,
  ...
  try {
    ...
    return res.success(messages.adLocation);
  } catch (err) {
    return next(err);
  }
};
...
const getAdDetails = async (req, res, next) => {
  try {
    ...
    return res.success(messages.adLocationsFetched, formatAd);
  } catch (error) {
    return next(error);
  }
};

const myAds = async (req, res, next) => {
  try {
    ...
    return res.success(messages.myadsFetched, formattedAds);
  } catch (error) {
    return next(error);
  }
};
...
const getRecentUnsavedPost = async (req, res, next) => {
  try {
    const userId = req.user.id;
    const ad = await Ad.findOne({
      where: {
    ...
    if (!ad) {
      return res.sucess(messages.noUnsavedPost);
    }
    const formattedAd = await formatAd(ad, { includeUser: false });
    return res.success(messages.unsavedAds, formattedAd);
  } catch (error) {
    return next(error);
  }
};
...
const searchCategories = async (req, res, next) => {
  try {
    ...
    return res.success(messages.searchCategories, result);
  } catch (error) {
    return next(error);
  }
};

const recommentedPosts = async (req, res, next) => {
  try {
    ...
    //   data: formattedAds,
    // });
    res.success(messages.recommentedPosts, {
      data: formattedAds,
      pagination,
    });
  } catch (error) {
...
const getAllPosts = async (req, res) => {
  ...
  return res.success(messages.allAds, posts);
};
...
const searchAds = async (req, res, next) => {
  try {
    ...
    let adsQuery = {
      ...
      include: [
        ...
        { model: AdImage, as: "ad_images" },
        {
          ...
          where: {
            ...(min_price !== undefined
              ? { rent_price: { [Op.gte]: Number(min_price) } }
              : {}),
            ...(max_price !== undefined
              ? {
                  rent_price: {
    ...
    //   data: formattedAds,
    // });
    return res.success(messages.searchAds, {
      list: formattedAds,
      pagination,
    });
  } catch (error) {
...
const rentCategoryPosts = async (req, res, next) => {
  try {
    const {
      ad_type,
      location_type,
    ...
    return res.success(messages.rentCategoryPosts, response);
  } catch (error) {
    return next(error);
  }
};

const bestServiceProviders = async (req, res, next) => {
  try {
    const perPage = 10;
    const {
      location_type,
    ...
    //   data: formattedAds,
    // });
    return res.success(messages.bestServiceProviders, {
      list: formattedAds,
      ...formatPagination({
        page: Number(page),
        perPage,
...
const adCategoriesFor = async (req, res, next) => {
  try {
    const adCategoriesArray = [];
    return res.success(messages.adCategories, adCategoriesArray);
  } catch (error) {
    return next(error);
  }
};
...
const addToWishlist = async (req, res) => {
  try {
    ...
    if (wishList) {
      ...
      //     .json({ success: true, message: "Wishlist removed" });
      ...
      return res.success(messages.wishlistAdded);
    }
  } catch (error) {
    return next(error);
  }
...
const changeOnlineStatus = async (req, res, next) => {
  try {
    const { ad_id } = req.body;
    const ad = await Ad.findOne({ where: { ad_id } });
    if (!ad) {
    ...
    return res.success(messages.adStatusChange)
  } catch (error) {
   return next(error);
  }
};
