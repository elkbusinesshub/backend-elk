            ...
            if (user) {
                ...
                if (referralCode && referralCode !== "") {
                    ...
                    if (referrerId === user.user_id) {
                        return res.status(responseStatusCodes.badRequest).json({ message: responseMessages.referralError });
                    }
                    if (referralOwner) {
...
exports.addReferralLogin = async (req, res) => {
    try {
        ...
        if (!referralOwner) {
            return res.status(responseStatusCodes.badRequest).json({ message: responseMessages.invalidReferralCode });
        }
        const referrerId = referralOwner.user_id;
...
exports.sendOtp = async (req, res) => {
    try {
        ...
        let otp;
        if (mobile == "9999999999" || mobile == "919999999999"|| mobile == "+91 9999999999") {
            otp = 123456;
        } else {
        ...
        res.json({ message: responseMessages.otpSend, verificationId: verificationId });
    } catch (error) {
...
exports.verifyOtp = async (req, res) => {
    try {
        ...
        if (!otpRecord) {
            return res.status(responseStatusCodes.badRequest).json({ message: responseMessages.InvalidOtp });
        }
...
exports.getUserById = async (req, res) => {
    ...
    try {
        ...
        if (!user) {
            return res.status(responseStatusCodes.badRequest).send({ message: responseMessages.userNotFound});
        }
        let profileUrl;
        if (user.profile) {
            profileUrl = getImageUrlPublic(user.profile);
        ...
        res.status(responseStatusCodes.internalServerError).send({ message: responseMessages.internalServerError });
    }
};
...
exports.updateProfilePic = async (req, res) => {    
    ...
    try{
        ...
        res.status(responseStatusCodes.internalServerError).json({ success: false, message: responseMessages.internalServerError });
    }
};
...
exports.updateEmailOrMobile = async (req, res) => {
    try {
        ...
        if (!user) {
            return res.status(responseStatusCodes.badRequest).json({ success: false, message: responseMessages.userNotFound });
        }
        if (mobile) {
            user.mobile_number = mobile;
            await user.save();
            return res.json({ success: true, message: responseMessages.mobileUpdated });
        }
        if (email && uid) {
            user.email = email;
            user.email_uid = uid;
            await user.save();
            return res.json({ success: true, message: responseMessages.emailUpdated });
        }
        return res.status(responseStatusCodes.badRequest).json({ success: false, message: responseMessages.invalidRequest });
    } catch (error) {
...
exports.updateProfile = async (req, res) => {
    ...
    try{
        ...
        await user.save();
        return res.status(responseStatusCodes.success).json({ success: true, message: responseMessages.profileUpdateSuccessfully });
    } catch (error) {
        return res.status(responseStatusCodes.internalServerError).json({ success: false, message: responseMessages.internalServerError });
    }
};
...
exports.deleteAccount = async (req, res) => {
    ...
    try {
        ...
        return res.status(responseStatusCodes.success).json({ success: true, message: responseMessages.userDeleted });
    } catch (error) {
        return res.status(responseStatusCodes.internalServerError).json({ success: false, message: responseMessages.internalServerError });
    }
};
...
exports.updateNotificationToken = async (req, res) => {
    try {
        ...
        if (!user) {
            return res.status(responseStatusCodes.badRequest).json({ message: responseMessages.userNotFound });
        }
...
exports.viewContact = async (req, res) => {
    try {
        ...
        return res.status(responseStatusCodes.success).json({ message: responseMessages.userDetails, data: response });
    } catch (error) {
        return res.status(responseStatusCodes.internalServerError).json({ message: responseMessages.internalServerError });
    }
};
