...
const getAdminAds = async (req, res) => {
    try {
        ...
        console.error(error);
        return res.status(responseStatusCodes.internalServerError).json({ success: false, message: 'Server error', error: error.message });
    }
};
...
const blockUserById = async (req, res) => {
    try {
        ...
        return res.status(responseStatusCodes.success).json({ success: true, message: 'User blocked successfully' });
    } catch (error) {
...
const deleteAdminAd = async (req, res) => {
    try {
        const { id } = req.query;
        if (!id) {
            return res.status(responseStatusCodes.badRequest).json({ success: false, message: "Ad ID is required" });
        }
        ...
        return res.status(responseStatusCodes.success).json({ success: true, message: "Ad deleted successfully" });
    } catch (error) {
        console.error(error);
        return res.status(responseStatusCodes.internalServerError).json({ success: false, message: "Server error", error: error.message });
    }
