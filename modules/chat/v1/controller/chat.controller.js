...
exports.addChat = async (req, res, next) => {
  ...
  try {
    ...
    if (file) {
      ...
      if (!uploaded) {
        return res.error(messages.fileUploadFailed);
      }
    }
    var chatMessage;
    if (type == "text" || type == "system") {
      ...
      if (file) {
        ...
        chatMessage.dataValues.file_url =
          file_name !== "" ? await getImageUrl(file_name) : null;
      } else {
        return res.error(messages.invalidRequest);
      }
    }
    // res
    ...
    //   });
    res.success(messages.chatAdded, chatMessage.dataValues);
  } catch (error) {
    return next(error);
  }
};
...
exports.blockAUser = async (req, res, next) => {
  ...
  try {
    ...
    if (existingBlock) {
      return res.success(messages.blockUser);
    }
...
exports.unblockAUser = async (req, res) => {
  ...
  try {
    ...
    if (result === 0) {
      ...
      return res.error(messages.noBlockRecord);
    }
    // return res
    //   .status(responseStatusCodes.success)
    //   .json({ message: "User unblocked successfully!" });
    return res.success(messages.userUnblocked);
  } catch (e) {
    return next(error);
  }
};
...
exports.isUserBlocked = async (req, res, next) => {
  ...
  try {
    ...
    if (isBlocked) {
      ...
      return res.success(messages.userBlocked, { blocked: true });
    } else {
      //   return res
      //     .status(responseStatusCodes.success)
      //     .json({ message: "User is not blocked", blocked: false });
      return res.success(messages.userNotBlocked, { blocked: false });
    }
  } catch (e) {
    return next(error);
  }
...
exports.getChatMessages = async (req, res, next) => {
  ...
  try {
    ...
    if (!chatRoom) {
      ...
      return res.error(messages.chatRoomNotFound, data);
    }
    ...
    await Promise.all(
      cleanMessages.map(async (message) => {
        message.file_url =
          message.file_name !== ""
            ? await getImageUrl(message.file_name)
            : null;
      })
    );
    data = {
    ...
    return res.success(messages.chatRoomFound, data);
  } catch (error) {
    return next(error);
  }
};
...
exports.getTotalChatRoomsCount = async (req, res, next) => {
  ...
  try {
    ...
    return res.success(messages.chatRoomFound, count);
  } catch (e) {
    return next(error);
  }
};
      ...
      data = await Promise.all(
        chatRooms.map(async (chatRoom) => {
          ...
          authUser.profile = authUser.profile
            ? await getImageUrl(authUser.profile)
            : null;
          otherUser.profile = otherUser.profile
            ? await getImageUrl(otherUser.profile)
            : null;
          return {
            ...chatRoom.toJSON(),
...
exports.deleteAllMessagesForUser = async (req, res, next) => {
  ...
  try {
    ...
    if (!chatRoom) {
      ...
      return res.error(messages.chatRoomNotFound);
    }
    const chatMessages = await ChatMessage.findAll({
      where: {
        room_id: chatRoom.room_id,
    ...
    return res.success(messages.deletedUserChat);
  } catch (error) {}
};
...
exports.deleteRoom = async (req, res, next) => {
  ...
  try {
    ...
    if (!chatRoom) {
      return res.success(messages.chatRoomAlreadyDeleted);
    }
    ...
    return res.success(messages.chatRoomDeleted);
  } catch (error) {
    return next(error);
  }
};
...
exports.deleteMessage = async (req, res, next) => {
  ...
  try {
    ...
    if (!message) {
      ...
      return res.success(messages.chatAlreadyDeleted);
    }
    await ChatMessage.destroy({ where: { id: id } });
    // return res
    //   .status(responseStatusCodes.success)
    //   .json({ message: "Successfully Deleted!" });
    return res.messages(messages.chatDeleted);
  } catch (error) {
    return next(error);
  }
};
