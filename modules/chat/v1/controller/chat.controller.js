    ...
    try {       
        ...
        if(type==='text'||type==='system'){
            ...
            if(file){
                ...
                getImageUrlPublic(file_name):null;
            }else{
                return res.status(responseStatusCodes.badRequest).json({ message: responseMessages.invalidRequest });
            }
        }
...
exports.reportAUser = async (req,res) => {
    ...
    try{
        await ReportUser.create({ reporter_id: authUserId, reported_id: otherUserId, reason: reason });
        return res.status(responseStatusCodes.success).json({ message: 'Success!' });
    }catch(e){
        return res.status(responseStatusCodes.internalServerError).json({ message: responseMessages.internalServerError });
    }
...
exports.getChatMessages = async (req, res) => {
    ...
    try {
        ...
        await Promise.all(cleanMessages.map(async (message) => {
            message.file_url = (message.file_name!=='')?getImageUrlPublic(message.file_name):null;
        }));        
        data={
...
exports.getTotalChatRoomsCount = async (req, res)=>{
    ...
    try{
        ...
        res.status(responseStatusCodes.internalServerError).json({ message: 'Something went wrong: ' + e.message });
    }
}
        ...
        if (chatRooms.length > 0) {
            data = await Promise.all(
                chatRooms.map(async (chatRoom) => {
                    ...
                    authUser.profile = authUser.profile ? getImageUrlPublic(authUser.profile) : null;
                    otherUser.profile = otherUser.profile ? getImageUrlPublic(otherUser.profile) : null;
                    const isBlockedByOther = await BlockedUser.findOne({
                        where: {
      ...
      data1 = await Promise.all(
        chatRooms.map(async (chatRoom) => {
          try {
            ...
            const authUser = chatRoom.User1.user_id === authUserId
              ? chatRoom.User1.toJSON()
              : chatRoom.User2.toJSON();
            const otherUser = chatRoom.User1.user_id === authUserId
              ? chatRoom.User2.toJSON()
              : chatRoom.User1.toJSON();
...
exports.deleteOneChatMessageForUser = async (req, res) => {
    ...
    try {
        ...
        res.status(responseStatusCodes.internalServerError).json({ message: 'Something went wrong: '  });
    }
};
